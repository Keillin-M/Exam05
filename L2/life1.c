...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/04/27 22:46:41 by kmaeda            #+#    #+#             */
/*   Updated: 2026/04/27 22:46:45 by kmaeda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct { int y, x, draw; } Pen;
...
int count_neighbor(char **b, int y, int x, int w, int h) {
	int c = 0;
	for (int dy = -1; dy <= 1; dy++) {
		for (int dx = -1; dx <= 1; dx++) {
			if ((dx || dy) && dy + y >= 0 && dx + x >= 0 && dy + y < h && dx + x < w && b[dy + y][dx + x] == 1)
			c++;
		}
	}
	return c;
...
void iter_game(char ***b, int w, int h) {
	...
	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
			int n = count_neighbor(*b, y, x, w, h);
			if ((*b)[y][x] == 1 && (n == 2 || n == 3))
				nb[y][x] = 1;
			else if ((*b)[y][x] == 0 && n == 3)
				nb[y][x] = 1;
...
int main(int argc, char **argv) {
	...
	while (read(0, &cmd, 1) > 0) {
		if (cmd == 'w' && p.y > 0)
			...
			p.draw = !p.draw;
		if (p.draw && (cmd == 'w' || cmd == 's' || cmd == 'd' || cmd == 'a' || cmd == 'x'))
			b[p.y][p.x] = 1;
	}
	for (int i = 0; i < it; i++)
		iter_game(&b, w, h);
